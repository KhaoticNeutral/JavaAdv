...
// the code uses a ThreadPool to execute multiple network requests concurrently, improving performance by utilizing multiple threads.
// The awaitTermination() method ensures that the main thread waits for all tasks to complete before proceeding.
// The callables map is used to track the results of each task and handle potential errors.